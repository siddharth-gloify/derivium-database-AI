...
Tests validate that all components wire together correctly.
"""
import pytest
...
class TestFullPipeline:
    def test_top_securities_by_issue_size(self):
        """Q12: top 10 by issue size — SQL and results flow end-to-end."""
        ...
        with patch("nltopgsql._get_client") as mock_llm:
        ...
        with patch("query_executor.get_connection", return_value=_db_rows(rows)):
    ...
    def test_psu_issuances_last_year(self):
        """Q2: PSU issuances last year — pipeline returns correct rows."""
        from nltopgsql import nl_to_sql
        from query_executor import execute_query
    ...
    def test_count_frb_returns_single_row(self):
        """Q11: count query returns a single-row count result."""
        from nltopgsql import nl_to_sql
        from query_executor import execute_query
    ...
    def test_maturing_next_month(self):
        """Q8: maturity date range query uses BETWEEN and CURRENT_DATE."""
        from nltopgsql import nl_to_sql
    ...
    def test_perpetual_bonds_with_call_before_2030(self):
        """Q7: perpetual + call date filter wired through correctly."""
        from nltopgsql import nl_to_sql
    ...
    def test_db_error_propagates(self):
        """A DB error from execute_query should bubble up to the caller."""
        from query_executor import execute_query
    ...
    def test_timing_both_components(self):
        """Both nl_to_sql and execute_query must return non-negative elapsed times."""
        from nltopgsql import nl_to_sql
        from query_executor import execute_query
